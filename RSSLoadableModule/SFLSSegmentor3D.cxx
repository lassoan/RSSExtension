...
//void CSFLSSegmentor3D::setSpacing(double dx, double dy, double dz)
//{
//    m_dx = fabs(dx);
//    m_dy = fabs(dy);
//    m_dz = fabs(dz);
...
This should not happen if the images are freshly loaded from file. Buy may happen when the input images are cropped from another image in memory.*/
    ...
    if (m_nx + m_ny + m_nz == 0)
    {
        ...
        std::cout<<"Loadable Spacing = ("<<m_dx<<", "<<m_dy<<", "<<m_dz<<")\n";
    }
...
void CSFLSSegmentor3D::oneStepLevelSetEvolution()
{
    ...
    /*--------------------------------------------------
    3.4 Scan Sn2     */
    for (CSFLSLayer::iterator itSn2 = Sn2.begin(); itSn2 != Sn2.end(); ++itSn2)
    {
